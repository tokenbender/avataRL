...
# start.sh - Run AvataRL training locally with torchrun
...
case $MODE in
    "single")
        if [ $GPUS -eq 1 ]; then
            ...
            python nanogpt/avatarl.py --compile=False
        else
            # Multi-GPU single node
            echo "Running on $GPUS GPUs (single node)..."
            torchrun --standalone --nproc_per_node=$GPUS nanogpt/avatarl.py
        fi
        ;;
    ...
    *)
        ...
        echo "Usage: ./start.sh [num_gpus] [mode]"
        echo "  num_gpus: Number of GPUs to use (default: 1)"
        echo "  mode: single, master, or worker (default: single)"
        echo
        echo "Examples:"
        echo "  ./start.sh              # Single GPU"
        echo "  ./start.sh 4            # 4 GPUs on single node"
        echo "  ./start.sh 8 master     # Master node with 8 GPUs"
